...
rp = eigen(1/dist(m,upper=TRUE,diag=TRUE))
dm = as.matrix(dist(m,upper=TRUE,diag=TRUE))
as.vector(cp$y)
rp$vectors[,1]
...
if (1 == 0){
  ...
  cp = CalcEigenCppD(Xs=mat,y=rep(1,dim(mat)[1]))
  rp = eigen(1/dist(mat,upper=TRUE,diag=TRUE))
  dm = as.matrix(dist(mat,upper=TRUE,diag=TRUE))
  as.vector(cp$y)
  rp$vectors[,1]
